        ...
        database.forEach(data => {
            if(item['barcode'] == data['barcode']){
                if(promotionBarcode.includes(item) && item.num >= 2 ){
                    let tempsum1 = parseFloat(data['price']) * (parseFloat(item['num']) - 1.0);
                    ...
                    receipt.push({'name':data['name'],'num':item['num'],'price':data['price'],'unit':data.unit,'tempsum':data['price'] * item['num'],'Itemdiscount':0,});
                }
            }
        })
    })
...
function printReceipt(tags) {
    ...
    const promotionBarcode = promotionItem.map(item => item.barcodes);
